...
void Lab8::Init()
{
    ...
    spotlightOn = false;
    spotCutoff = 20.0f;    // grade
    spotExponent = 10.0f;  // smoothness
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	glUniform3f(light_position_2, lightPosition2.x, lightPosition2.y, lightPosition2.z);

    // Spotlight uniforms
    int is_spot = glGetUniformLocation(shader->program, "is_spotlight");
    glUniform1i(is_spot, spotlightOn ? 1 : 0);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        // Move second light with IJKL U O (example)
